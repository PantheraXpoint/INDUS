...
def extract_frames_from_events(
    ...
    for event in events:
        ...
        if duration is None:
            continue
        frame_numbers.extend(range(int(duration[0]*video_fps), int(duration[1]*video_fps) + 1))
    if max_frames is None:
        frame_interval = max(1, int(video_fps / target_fps))  # Frames to skip between samples
    else:
        frame_interval = max(1, math.ceil(len(frame_numbers) / max_frames))
    extracted_frames = []
    
    for frame_number in frame_numbers[::int(frame_interval)]:
...
def main():
    ...
    output_dir = "iterative_pruning_output"
    ...
    base_path = Path("SAE-k2-obj_filter")
    base_folders = [base_path / folder for folder in os.listdir(base_path)]
    for base_folder in base_folders:        
        video_path = f"AVA100/videos/{base_folder.name}.mp4"
