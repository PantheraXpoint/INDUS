#!/usr/bin/env python3
"""
Run ECML-PKDD/calc_retrieval_accuracy.py on top_k_frames_retrieval output.

Frames retrieval writes { "results": [...], "num_queries", "average_latency_seconds_per_query" } under
out_<dataset>/seed_events_<dataset>.json. calc_retrieval_accuracy expects a JSON file whose root is
a list of entries (video_key, question_id, seed_events). This script extracts "results" and writes
a list-only JSON, then invokes calc_retrieval_accuracy without modifying it.
...
def main():
    parser = argparse.ArgumentParser(
        description="Run retrieval accuracy on top_k_frames_retrieval output (converts results wrapper to list)."
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to frames retrieval seed_events JSON (default: script_dir/out_<dataset>/seed_events_<dataset>.json)",
    )
    parser.add_argument(
        "--project-root",
        type=Path,
        default=None,
        help="Project root (default: parent of top_k_frames_retrieval)",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--keep-converted",
        action="store_true",
        help="Keep the converted list-only JSON file (default: delete after run)",
    )
    args = parser.parse_args()

    seed_events_path = args.seed_events or (script_dir / f"out_{args.dataset}" / f"seed_events_{args.dataset}.json")
    ...
    out_list_path = script_dir / f"out_{args.dataset}" / f"seed_events_{args.dataset}_for_calc.json"
