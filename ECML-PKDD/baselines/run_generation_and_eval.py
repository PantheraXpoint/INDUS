...
def _call_model_get_usage(model, batch_inputs: List[Dict]) -> Tuple[str, Optional[Dict]]:
    ...
    """
    try:
        result = model.batch_generate_response(batch_inputs, return_usage=True)
    except TypeError:
    ...
    for idx, entry in enumerate(results_list):
        ...
        try:
            ...
            if response and response.strip().startswith("[ERROR]"):
                if idx == 0:
                    print(f"[server error] First request failed: {response.strip()}")
        except Exception as e:
            response, usage = "", None
...
def run_frames_retrieval(
    ...
    gt_cache = load_gt_answers(dataset, project_root)
    mt = model_type if vlm_model == "qwenvl_vllm" else None
    vlm = init_model(vlm_model, num_gpus=gpus, port=port, model_type=mt)
...
def run_ava_extract(
    ...
    """Single query, one mode. Returns (response_entry, eval_entry)."""
    ...
    if mode == "frames_only":
        if not frames or not frame_meta:
            ...
            mt = model_type if vlm_model == "qwenvl_vllm" else None
            vlm = init_model(vlm_model, num_gpus=gpus, port=vlm_port, model_type=mt)
            try:
        ...
        if not seed_events:
            ...
            mt = model_type if llm_model == "qwenvl_vllm" else None
            llm = init_model(llm_model, num_gpus=gpus, port=llm_port, model_type=mt)
            try:
                response, usage = _call_model_get_usage(llm, [{"text": prompt}])
                response = response or ""
            except Exception as e:
...
def main():
    ...
    parser.add_argument("--vlm-model", type=str, default="qwenvl_vllm",
                        help="Use qwenvl_vllm to connect to hosted server (no local load)")
