"""
Uniform Sampling Query - Baseline VLM inference without retrieval
Uniformly samples frames from entire video
"""
from AVA.uniform_sampling import UniformSampling
import os
import json
import fcntl
import time
...
def setup_profiling_logger(output_folder, process_num):
    ...
    log_file = os.path.join(output_folder, f"profiling_uniform_sampling_process_{process_num}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    logger = logging.getLogger(f'profiling_uniform_sampling_process_{process_num}')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Uniform Sampling Query - Baseline VLM inference")
    ...
    # Sampling strategy parameters (mutually exclusive)
    parser.add_argument("--target_fps", type=float, help="Target FPS for uniform sampling")
    parser.add_argument("--max_frames", type=int, help="Maximum number of frames to sample")
    
    # Batching parameter
    parser.add_argument("--batch_size", type=int, default=4, help="Number of questions per VLM batch")
    
    # Other parameters
    parser.add_argument("--use_cache", action="store_true", help="Enable frame caching")
    
    args = parser.parse_args()
    
    # Validate sampling parameters
    if args.target_fps is None and args.max_frames is None:
        print("Error: Either --target_fps or --max_frames must be specified")
        exit(1)
    
    if args.target_fps is not None and args.max_frames is not None:
        print("Error: Cannot specify both --target_fps and --max_frames")
        exit(1)
    ...
    if args.video_id != -1:
        ...
        profiler_logger.info("=== STARTING SINGLE VIDEO PROCESSING (Uniform Sampling) ===")
        
        # Log sampling parameters
        if args.target_fps:
            profiler_logger.info(f"Sampling strategy: target_fps = {args.target_fps}")
        else:
            profiler_logger.info(f"Sampling strategy: max_frames = {args.max_frames}")
        ...
        # UniformSampling object creation
        uniform_start = time.time()
        uniform_sampling = UniformSampling(video=video, llm_model=vlm)
        uniform_end = log_timing(profiler_logger, "UniformSampling Object Creation", uniform_start)
        
        # Answer generation
        answer_start = time.time()
        result = uniform_sampling.generate_answer(
            question=question,
        ...
        profiler_logger.info("=== STARTING BATCH PROCESSING (Uniform Sampling) ===")
        
        # Log sampling parameters
        if args.target_fps:
            profiler_logger.info(f"Sampling strategy: target_fps = {args.target_fps}")
            sampling_suffix = f"_{args.target_fps}fps"
        else:
            profiler_logger.info(f"Sampling strategy: max_frames = {args.max_frames}")
            sampling_suffix = f"_{args.max_frames}frames"
        
        profiler_logger.info(f"Batch size: {args.batch_size} questions per batch")
        
        # Output file
        json_file = f"{output_folder}/query_uniform_sampling_{args.dataset}_{args.model}{sampling_suffix}_process{args.process_num}.json"
        ...
        current_batch = []
        current_batch_metadata = []  # (video_id, question_id, question_data, uniform_sampling_obj)
        ...
        for video_id in range(start_video, end_video + 1):
            ...
            try:
                ...
                # Create UniformSampling object once per video
                uniform_creation_start = time.time()
                uniform_sampling = UniformSampling(video=video, llm_model=vlm)
                uniform_creation_end = log_timing(profiler_logger, f"Video {video_id} UniformSampling Creation", uniform_creation_start)
            ...
            for question_id in range(len(qas)):
                ...
                try:
                    ...
                    frames, sampling_info, overlap_info = uniform_sampling.prepare_question_data(
                    ...
                    # Create prompt
                    prompt = uniform_sampling.create_prompt(question_data["question"])
                    ...
                    if len(current_batch) >= args.batch_size:
                        ...
                        for (vid, qid, qdata, samp_info, overlap_info), response in zip(current_batch_metadata, responses):
                            result = {
                                "video_id": vid,
                                "question_id": qid,
